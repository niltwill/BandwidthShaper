...
WinDivert itself should support the most recent Windows operating systems, starting from Windows 7 and Windows Server 2012. It does not support older Windows versions like Windows XP or Windows Vista. For Windows XP, you can use [Traffic Shaper XP](https://www.majorgeeks.com/files/details/traffic_shaper_xp.html) instead.

### Background Info
Why was this made? The reason being is that most apps that serve this purpose can be rather complex (with many extra features) and they are usually commercial in Windows, such as [NetLimiter](https://www.netlimiter.com), [NetBalancer](https://netbalancer.com) or [SoftPerfect Bandwidth Manager](https://www.softperfect.com/products/bandwidth). At best, you can use [TMeter](http://www.tmeter.ru/en) as a free option. None of these were really appealing to me for simple bandwidth throttling (and I had no other extra need!). Now, if you ever had to use an ADSL/VDSL connection, you will know how much the upload saturation sucks, even with a smaller file. Browsing a simple website also becomes a nightmare, as if you had a very poor connection. So I needed the most lightweight and portable option to throttle my upload speed, and that's about it. I couldn't be bothered with router or firewall shenanigans, and a quick "on or off" via a simple tool was what I required.
...
## Usage
First you should use the `--list-nics` option to print all the network interfaces and to check out which interface(s) you want to throttle. Remember its index number and use that for the `-n` parameter. If you want to use multiple network interfaces (e.g. wired and wireless), you can separate the index numbers with a comma. It is not yet possible to set different global speed rate limits for different network interfaces (this may come in a later version). Also, using virtual adapters here will most likely break your network connection, so I recommend to use only your physical interfaces.
...
## Examples
> BandwidthShaper.exe --nic 15 -p chrome.exe -i 300000t -d 250 -u 100\
> BandwidthShaper.exe -q -n 15 -d 1MB -u 0.5MB

Because this uses the WinDivert driver on demand, the application must have administrator privileges! So make sure to run it from an admin CMD or PowerShell. The supplied batch file (`run-BandwidthShaper.bat`) should open the right executable depending on your architecture (x86 or x64). All you need to do before that is to check out the options before and use your correct NIC index for your network interface(s), as well as edit the parameters to what you need.

If you do not always want to open an elevated command prompt and manually execute the app with its flags, then it's best to set your preferred flags in `run-BandwidthShaper.bat` and use that for the time being. Remember: make sure to change the NIC's index number to be valid, yours will most likely need to be different from mine!
